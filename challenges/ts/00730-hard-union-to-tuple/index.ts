...
type UnionToTuple<T> = UnionToIntersection<
  (T extends T ? () => T : never)
> extends () => infer R  // R is a single type
  ? [...UnionToTuple<Exclude<T, R>>, R]
  : [];

type Foo<T> = T extends T ? () => T : never;
type result = Foo<string | number>  // (() => string) | (() => number)
type result2 = UnionToIntersection<result> // (() => string) & (() => number)

type result3 = result2 extends () => infer R ? R : never; // number
type resultNever = UnionToIntersection<never> // never
